...
# ======================== 3. 筛选区美化（卡片式布局） ========================
with st.container():  # 卡片容器
    st.markdown("<h3 style='margin-bottom: 1rem;'>🔍 数据筛选</h3>", unsafe_allow_html=True)
    col1, col2, col3 = st.columns([1,1,1], gap="large")  # 增加列间距，更宽松
    ...
    with col1:
        province = st.selectbox(
            ...
            index=0,
            label_visibility="collapsed"  # 隐藏默认标签，自定义
        )
...
with st.container():
    ...
    st.map(
        ...
        longitude="lon",
        size="人口占比",
        color="#2e86ab",  # 更高级的蓝绿色
        zoom=4 if province == "全部" else 7
    )
st.divider()

# ======================== 5. 图表模块（双列布局+美化） ========================
col_chart1, col_chart2 = st.columns([2,1], gap="large")  # 柱状图占2/3，饼图占1/3，更协调
...
with col_chart1:
    with st.container():
        ...
        fig, ax = plt.subplots(figsize=(12, 6))  # 更大的图表，更清晰
        # 渐变配色（更高级）
